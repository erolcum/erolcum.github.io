---
layout: post
title: PureBasic ile Arduino -1- LED YAKMA
---

Purebasic ile hazırlayacağımız, arayüz veya kullanıcı formu ile Arduino'yu kontrol edeceğiz. Bir tane Arduino Uno alıp, bu dili öğrenmek daha faydalı ve eğlenceli olacaktır.

Seri bağlantı için Arduino'nun kendi usb kablosunu kullanacağız. Kablo dışında bir LED ve 220 ohm ile 1K arası bir dirence ihtiyaç olacak. Breadboard üzerinde bulunan her 5 delik kısa devredir, malzemeleri birbirine bağlamak için kullanılır. Şemadaki gibi bağlantı yaparsanız devre çalışmayacaktır. Arduino GND'den gelen kablo, direncin girdiği 5 delikten birine girmelidir. Ayrıca LED'i ters bağlarsanız yanmaz. Evet, bu şema için breadboard'a gerek yok ama başka şemalarda ihtiyacın olabilir.

Seri port, yeni bir dizüstü PC alınca göremeyeceğiniz, en eski bağlantı tipi olmasına rağmen halen fabrikalarda kullanılmaktadır. En yaygın seri port soketi 2 sıra, 9 pin olandır. Bağlantı yapacağınız cihaza göre pin bağlantıları değişebilir.
<br><br>
...
Programı test etmek için aslında Arduino'ya da ihtiyacın yok. Bu durumda Arduino'nun yaptığı işi senin yapman gerekecek. Eski blog sayfamda anlattığım [com0com](https://erolcum.blogspot.com/2022/12/null-modem-emulator-com0com-sanal-seri.html) sanal seri port programını kullanabilirsin.<br><br>
